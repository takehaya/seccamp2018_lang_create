...
type llvm_context = {
  ...
  declared_funcs : (string * (string list * exp_t * ty)) list;
  builder : llbuilder;
  func : llvalue;
  namespace : string;
}
...
let llvm_module = create_module llvm_ctx "region_mio"
...
let rec lltype_of_type ty =
  match ty with
  |IntTy -> i32_t
  |BoolTy -> bool_t
  |UnitTy -> i32_t
  |_ -> raise (MioError ("Unsupported type:"^(string_of_type ty)))
...
let rec codegen_defun fname arg_names types ret_t body ctx =
  ...
  let rec add_arg argnames argtypes params =
    ...
    |(argname::an, argtype::at, param::ps)->begin
      ...
      Hashtbl.add (List.hd ctx.env) argname store;
      add_arg an at ps
    end
    |([], [], []) -> ()
    |([], [void_t], [_]) -> ()
    |_ -> raise (MioError("Program Errors"))
  in
    ...
    |TVar(name, _) -> begin
      match lookup name ctx.env with
        |Some(v) ->
          ...
          (r, ctx)
        |None -> raise (MioError ("Undefined variable: " ^ name))
    end
    |TAssign(name , exp, t) ->
      let v, ctx = codegen_expr exp ctx in
      let store = 
        match t with
        |IntTy -> build_alloca i32_t name ctx.builder
        |BoolTy -> build_alloca bool_t name ctx.builder
        |UnitTy -> raise (MioError "Unit type has not value")
        |_ -> raise (MioError ("Unspported type: " ^ (string_of_type t)))
      in
      ...
      |None -> begin
        ...
        |Some(f_id) -> begin
          ...
          |Some(f) -> 
            ...
            (r, ctx)
          |None -> raise (MioError "Program Error: function id missed")
        end
        |None -> begin
          ...
          |Some(arg_names, body, ftype) ->
              ...
              let r = build_call f (Array.of_list args) "" ctx.builder in
              (r, ctx)
          |None -> raise (MioError ("undefined function (or does not match types): "^fname))
        end
      end
    end
    |TMultiExpr (exprs, _) ->
